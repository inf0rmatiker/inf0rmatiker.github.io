...
* Alerts for surges in API usage should be put in place
...
* Filtering should be used to limit returned data based on filter parameters
...
Filter operators can be sent as part of the URL request using a colon, such as: `?price=gte:10`.
...
* _Implicit Flow Grant Type_: Intended for clients such as single page web apps that can't protect client secrets. The authorization server returns an access token. This grant type does not return a refresh token, as the browser cannot keep it private.

...
manipulate the pointer yourself, then dereference:
...
Initializing them using curly braces like we did with single-dimension arrays:
...
C++ will allocate all this memory contiguously, as if it were a single-dimensional array, but it will keep track of the offsets for each sub-array.
